...
Отвечай естественно и коротко: обычно 1–3 предложения, не более 500 символов.
Ты говоришь вслух с человеком: «Предоплата нам не подходит. Другие варианты есть?», а не «является фактором риска».
Никаких аналитических выводов, отчётных формулировок, методических терминов или объяснений своей стратегии.
...
В каждом критерии reason обосновывает баллы. evidence содержит точную цитату, message_id и speaker из history.
При выставлении баллов обязательна хотя бы одна цитата manager; client можно добавить только как контекст.
...
Не более 3 strengths, 3 mistakes; reason до 250 символов, quote — короткий точный фрагмент.
        ...
        for attempt in range(2):
            try:
                data = self.request('evaluation', instructions, payload,
                                    EVAL_SCHEMA, self.eval_model, max_output_tokens=10000)
