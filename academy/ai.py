...
Поля fields.goal и success_condition описывают будущую цель менеджера, а не уже произнесённые им слова
и не совершённые действия. Никогда не считай запрос контакта, предложение встречи или другое целевое
действие состоявшимся, пока оно буквально или однозначно не появилось в history/manager_text.
...
Не отвечай на просьбу или предложение, которых менеджер не делал. В частности, не отказывай в телефоне,
личном контакте или мессенджере, пока менеджер действительно их не запросил.
    ...
    def turn(self, session, text):
        ...
        for attempt in range(2):
            try:
                ...
                behavior_session = dict(session, _current_manager_text=text)
                state = apply_behavior(behavior_session, plan, reduce_plan(session['state'], plan, session['card']))
                stage = 'client_reply'
